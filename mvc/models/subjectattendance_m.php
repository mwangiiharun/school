...
class Subjectattendance_m extends MY_Model {
	...
	function update_sub_attendance_classes_new($colname, $status, $schoolyearID, $classesID, $monthyear, $sectionID, $subjectID) {
		...
		$this->db->query("UPDATE $this->_table_name SET $colname='".$status."' WHERE schoolyearID = $schoolyearID AND classesID = $classesID AND monthyear = '".$monthyear."' AND sectionID = $sectionID AND subjectID = $subjectID AND ($colname !='L' OR $colname IS NULL)");
		return TRUE;
	}
